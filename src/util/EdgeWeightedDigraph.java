...
public class EdgeWeightedDigraph implements Serializable{
	...
	public EdgeWeightedDigraph(int l) {
        if (l < 0) throw new IllegalArgumentException("Number of vertices in a Digraph must be nonnegative");
        this.V = l;
        this.E = 0;
        adj = (Bag<DirectedEdge>[]) new Bag[l];
        for (int v = 0; v < l; v++)
    ...
    public EdgeWeightedDigraph(In in) {
        ...
        for (int i = 0; i < E; i++) {
            int v = in.readInt();
            int w = in.readInt();
//            System.out.println(" E=" + E + " V=" + V + " v=" + v + " w=" + w);
    ...
    public EdgeWeightedDigraph(In in, boolean ifEdge) {
	    this(in.readInt());
	    int E = in.readInt();
        if (E < 0) throw new IllegalArgumentException("Number of edges must be nonnegative");
        for (int i = 0; i < E; i++) {
...
//            System.out.println(" E=" + E + " V=" + V + " v=" + v + " w=" + w);
            ...
            if (w < 0 || w >= V) throw new IndexOutOfBoundsException("vertex " + w + " is not between 0 and " + (V-1));
            double weight = in.readDouble();
            addEdge(new DirectedEdge(id, v, w, weight));
        }
    }
    ...
    public EdgeWeightedDigraph(Map<Integer, Node> nodeMap, Map<Integer, Edge> edgeMap){
    	this(nodeMap.size());
        this.E = edgeMap.size();
        
        HashMap<Integer, Stack<DirectedEdge>> stackMap = new HashMap<Integer, Stack<DirectedEdge>>();
        for (Node node : nodeMap.values()) {
            stackMap.put(node.getId(), new Stack<DirectedEdge>());
        }
        
    	for(Edge e : edgeMap.values()){
    		int v = e.getStart();
    		Stack<DirectedEdge> reverse = stackMap.get(v);
    		reverse.push(new DirectedEdge(v, e.getEnd(), e.getWeight()));
    	}
    	
    	for(int v : stackMap.keySet()){
    		Stack<DirectedEdge> reverse = stackMap.get(v);
            for (DirectedEdge e : reverse) {
                adj[v].add(e);
            }
